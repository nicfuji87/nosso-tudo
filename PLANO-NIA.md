...
**Já entregue além da fundação:** cota mensal; injeção de `nia_contexto`; feedback 👍/👎 + análise conversacional; **adaptador OpenAI** (Chat Completions, incl. GPT-5/raciocínio) + gestão de preços por modelo; **ferramentas de leitura** (`consultar_cadastros`, `listar_transacoes`) e **de cadastro** (`criar_categoria/conta/cartao`); **prompt caching da Anthropic**; **streaming token-a-token (OpenAI e Anthropic)**; migrations 0007–0009 aplicadas ✓. Tudo em produção.

**Ainda não feito (próximas fatias):** widget `resolver_match` (zona cinza inline — hoje a resolução fina de estabelecimento só roda no WhatsApp/Inbox).
